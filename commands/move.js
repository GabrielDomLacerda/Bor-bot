...
module.exports = {
    ...
    execute: async function (interaction, eventType, parameters) {
        ...
        if (!queue) {
            return await ephemeralReply(interaction, 'Não estou tocando nada!', eventType)
        }
        const idMusic = eventType == Events.InteractionCreate? interaction.options.getInteger('id') : parameters.join(' ')
        if (idMusic < 1 || idMusic > queue.songs.length) {
            return await ephemeralReply(interaction, 'Música inválida', eventType)
        }
        ...
        return await ephemeralReply(interaction, `${movingSong.title} movida para a posição 1`)
    },
};
