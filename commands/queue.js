    ...
    data: new SlashCommandBuilder()
        ...
        .addIntegerOption((option) =>
            option
                .setName("pagina")
                .setDescription(
                    "Número da página da fila que será mostrado (default: 1)"
                )
        ),
        ...
        if (!serverQueue) {
            ...
            const pageIndex =
                eventType == Events.InteractionCreate
                    ? interaction.options.getInteger("pagina")
                    : parameters.join(" ");
            ...
            if (fullText.length <= maxLength) {
                return await interaction.reply(
                    queueTitles.reverse().join("\n")
                );
            } else {
                if (!pageIndex || pageIndex <= 0 || pageIndex == 1) {
                    let idxFinal = fullText
                        .slice(0, maxLength - 1)
                        .lastIndexOf("\n");
                    return await interaction.reply(
                        fullText
